...
** Change the `Square` property's type to `ICell<Square>`.
...
* XAML code
** Update the `Fill` binding. Since we upgraded property `Square` from type `Square` to `ICell<Square>`, the binding needs to change from `Square` to `Square.Value`.
...
[source,xml]
----
<Button Content="{Binding Square}" Width="32" Height="32" />
----

In other words, we tell the `Button` that its contents are equal to its `DataContext`, which itself is equal to a `Square`. But WPF is using the `ToString()` method to fill in the text, and per requirements strings passed from Model or ViewModel are not allowed. So lets draw something from Square ourselves.
...
To convert from `Square` to `Brush`, we need to create an `IValueConverter`. But `IValueConverter` only accepts one input while we require atleast two arguments: the state of the square and the amount of adjacent mines. We also can not give Square itself as input since it doesn't expose these properties. Instead we can use a `wrapper` class that exposes these 2 values as properties and pass an instance of that class to the converter.
...
public class SquareWrapper()
{
    ...
    public SquareWrapper(IGame game, int row, int column)
    {
        var position = new Vector2D(row, column);
        IsCovered = game.IsSquareCovered(position);
        IsMine = game.IsGameOver && game.Mines.Contains(position);
        IsFlagged = game.Flags.Contains(position);
        AdjacentMines = g.Board[new Vector2D(row, column)].AmountOfMinesNear;
    }
}
----
...
[NOTE,caption={task-caption}]
====
* In GameBoardSquareViewModel, change the type of property Square from `ICel<Square>` to `ICel<SquareWrapper>`
...
* Add three properties `Covered`, `Uncovered`, `Flagged` and `Mine`. Each has type `object` and has both a getter and setter.
* In the `Convert` method, return the values of `Covered`, `Uncovered`, `Flagged` and `Mine` instead of hardcoded brushes.

In `MainWindow.xaml`, initialize the three properties of the `SquareColorConverter`.
[source,diff]
----
  <Window ...>
      <Window.Resources>
...
* Create a second converter `SquareTextConverter` to fill in the `Text` of `TextBlock`.
====
