...
Divide the process in a series of minimal changes and recompile in between every step.

* Replace `IGame` by `ICell<IGame>`.
...
Run the code.
You should be able to uncover squares by clicking on their corresponding button.
Take your time to understand why it works.
====
